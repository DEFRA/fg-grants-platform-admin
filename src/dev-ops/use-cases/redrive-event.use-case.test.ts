...
describe('redriveEventUseCase', () => {
  ...
  // the button being pressed, and the state it is in now is the useful half.
  // The banner prints it as it stands, so what the body has to carry is the
  // word a human reads rather than the enum the wire is written in.
  test('reports a refused redrive as a conflict, in the words the backend sent', async () => {
  ...
  // A status this app has no sentence for is still worth naming, and the
  // endpoint's own spelling is the string worth grepping for.
  ...
  test('ignores a status the body did not send as a string', async () => {
    vi.mocked(redriveEvent).mockRejectedValue(
      responseError(409, { status: 7, statusLabel: 7 })
    )
