/**
 * Ambient module declarations for internal Defra packages and other
 * dependencies that do not ship their own TypeScript types. Typed only to
 * the surface area this codebase actually uses.
 *
 * This file must stay a global script (no top-level import/export) so that
 * `declare module` here registers fresh ambient types for these untyped
 * packages. Augmenting @hapi/hapi's own (already typed) module lives
 * separately in hapi-augmentation.d.ts, which does need to be a module.
 */
